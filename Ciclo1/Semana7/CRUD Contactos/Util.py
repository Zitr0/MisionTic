...
def agregarBarraEstado(ventana, totalEtiquetas):
    barraEstado = Frame(ventana)
    barraEstado.pack(side=BOTTOM, fill=X)
    etiquetas = []
    for i in range(totalEtiquetas):
        lbl=Label(barraEstado, text="", borderwidth=2, relief="groove")
